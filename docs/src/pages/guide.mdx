...
Use `scene.enableHatchFill(false)` to temporarily hide all geometry fills (including custom hatch fills), then `scene.enableHatchFill(true)` to show them again.
