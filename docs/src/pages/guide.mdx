...
I created a very basic class that exposed primitives like `moveTo`, `lineTo` and `arcTo` and things started to get more
very interesting. At this point I was hooked in.
...
readability, and no hidden magic as core values.
...
scene.draw((p) => {
  ...
  circle
    ...
    .close(); // optional, the circles is basically close, this finalizes the path
});
```
...
scene.draw((p) => {
  ...
  circle
    .moveToAbs(-radius, 0)
    .arcTo(0, -Number.EPSILON, radius, true, true) // draw a large arc from wher we are to almost where we are, clockwise
    .close();
});
```
...
My personally advice is to use at least two arcs because it's much more robust, don't be lazy :)
...
`scene.enableFilter(true)` applies an SVG displacement filter that gives geometry a hand-drawn look.
`scene.setFilterIntensity(value)` controls how strong the effect is.
...
```typescript
scene.enableFilter(true); // pencil/noise effect, default: false
