...
| `setFilterIntensity(number)` | Set pencil displacement intensity (default: `1.25`). Callable inside or outside draw callbacks. |
...
| `enableFill(bool)`      | Toggle visibility of geometry fills (including custom `addHatchFill` fills). Does not assign hatch fills by itself. |
...
| `scale(x, y)`        | Scale the entire group. Direct DOM write - no redraw needed.                                                          |
